-- [A11] Profilul nu mai are coloana email.
--
-- Contul se tine pe numarul de telefon: adresa din auth.users este facuta din
-- numar, este interna si omul nu o vede niciodata. identitate.profiluri.email
-- a ramas din lumea de dinainte, scrisa mereu null de la_cont_nou, cu un
-- comentariu care spunea "adresa cu care intra in cont" - adica exact ce nu
-- mai este adevarat. O coloana goala pe care nimeni nu o scrie si nimeni nu o
-- citeste nu are ce cauta in schema.
...
as $$
begin
  insert into identitate.profiluri (id, nume, telefon)
  values (
    new.id,
...
begin
  ...
  update identitate.profiluri
     set nume = 'Persoana stearsa', telefon = null
...
alter table identitate.profiluri drop column email;
