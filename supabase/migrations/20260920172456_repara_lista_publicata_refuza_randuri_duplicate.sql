-- intretinere.salveaza_lista_publicata(): verificarea "cheltuiala e impartita
-- corect" numara randurile cu count(t.apartament_id), fara distinct. Un
-- rezultat cu un apartament dublat si altul lipsa poate insuma exact suma
-- cheltuielii si avea exact atatea randuri cate apartamente are blocul, asa
-- ca verificarea trecea; abia INSERT-ul cadea pe cheia unica
-- repartizari_cheltuiala_apartament_versiune_key, cu eroarea bruta de
-- Postgres in loc de mesajul "nu este impartita corect" ([NOU-1] din
-- b-intretinere-publicare.test.sql).
--
-- Fix: numaram apartamentele distincte, nu randurile; un rand dublat pentru
-- acelasi apartament face ca numarul de apartamente distincte sa nu mai
-- ajunga la numarul de apartamente din bloc (unul lipseste in schimb), sau
-- ramane sub numarul total de randuri daca niciun apartament nu lipseste
-- (duplicat curat). Ambele cazuri cad acum pe verificarea existenta.
...
begin
  ...
  -- fara apartamente lipsa sau dublate.
  select count(*) into v_nr_apartamente from organizare.apartamente where bloc_id = v_lista.bloc_id;
  for v_gresita in
    select c.cod, c.suma, coalesce(sum(t.suma), 0) as repartizat,
           count(t.apartament_id) as randuri, count(distinct t.apartament_id) as apartamente
    ...
    having coalesce(sum(t.suma), 0) <> c.suma
        or count(distinct t.apartament_id) <> v_nr_apartamente
        or count(t.apartament_id) <> count(distinct t.apartament_id)
  loop
    raise exception 'Cheltuiala % nu este impartita corect: % lei din %, pe % apartamente din %.',
      v_gresita.cod, v_gresita.repartizat, v_gresita.suma, v_gresita.apartamente, v_nr_apartamente;
  end loop;
  if exists (
